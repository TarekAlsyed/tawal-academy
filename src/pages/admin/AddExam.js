...
const AddExam = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(API_ENDPOINTS.ADMIN_EXAMS(subjectId), formData);
      
      if (response.data.success) {
        toast.success('تم إنشاء الامتحان بنجاح');
        navigate(`/admin/subjects/${subjectId}/exams`);
      }
    } catch (error) {
    ...
    <AdminLayout>
      <div className="admin-content">
        <div className="page-header">
          ...
          <button
            className="btn-back"
            onClick={() => navigate(`/admin/subjects/${subjectId}/exams`)}
          >
            العودة
          </button>
        </div>
