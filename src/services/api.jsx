...
// ⚡ Cache fingerprint for the entire session (canvas + SHA256 is expensive on Android WebView)
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // ⚡ CRITICAL: _noRetry flag = absolutely no retries (used by auth checks)
    if (config?._noRetry) {
...
api.interceptors.request.use(
  (config) => {
    // ⚡ CRITICAL: If _noRetry is set, do NOT override retry/timeout settings
    // This ensures auth checks stay fast and never retry
    if (!config._noRetry) {
    ...
    // ⚡ Use cached fingerprint (avoids canvas+SHA256 on every request = 200-500ms saved)
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    // ⚡ Auth check requests (_noRetry) should NOT trigger token refresh
    // They are quick probes — if they get 401, let the caller handle it
