...
export const initializeNativeBridge = async () => {
  if (!isNativePlatform()) {
    return; // Silent skip on web — no console noise
  }
  
  // ⚡ CRITICAL FIX: Run ALL native initializations in PARALLEL
  // Old code: 5 sequential awaits = ~2500ms
  // New code: all parallel = ~500ms (limited by slowest single call)
  await Promise.allSettled([
    hideSplashScreen(),           // Hide splash ASAP
    configureStatusBar(true),     // Dark mode default
    setupKeyboard(),
    setupBackButton(),
    setupAppStateListener(
      () => {}, // App resumed — handled by visibility change in AuthContext
      () => {}  // App paused
    ),
  ]);
};
